...
Target meter defaults to `http://homewizard.local`. Override with `P1_METER_URL`.

Before changing collector, dashboard, deployment, or health-check behavior,
check `docs/USER_STORIES.md` for the user-level contract and acceptance
criteria. Update that doc intentionally when product behavior changes.
...
`kasa_monitor.py` handles discovery and polling via `python-kasa` (IOT protocol, TCP 9999). Subnets to scan come from `KASA_SUBNETS` (comma-separated CIDR list). Discovery uses direct `Discover.discover_single(ip)` probes over the whole /24 with asyncio concurrency 50, because UDP broadcast may not traverse routed subnet boundaries.
...
### Tuya / Nedis smart plug integration

`tuya_monitor.py` handles Tuya-family plugs (including many Nedis SmartLife plugs) via `tinytuya` on local TCP `6668`. Local device IPs/MACs live only in gitignored `devices.json` / `tuya_devices.json`.

Live polling requires a Tuya `devices.json` file generated by `tinytuya wizard` with the device's local key. Optional local overrides and placeholder candidates live in `tuya_devices.json`. Both files are gitignored, along with `tinytuya.json`, `snapshot.json`, and `tuya-raw.json`.

Typical one-time setup:

```bash
./p1_env/bin/python -m tinytuya wizard -region eu
```

If the plug uses nonstandard DPS indexes for power/current/voltage, add overrides in `tuya_devices.json` (`power_dps`, `power_scale`, etc.). Without a local key, the dashboard can still show the candidate device, but it cannot decrypt live wattage.

## Gotchas

- `web_monitor.py` auto-starts background collection at import-time of `__main__`. The `/start_monitoring` and `/stop_monitoring` endpoints also toggle the same `monitoring_active` global. P1, Kasa, Tuya, and MELCloud polling threads all check this flag.
- Collection interval hard-coded to 60s in `background_data_collection`, `background_kasa_collection`, and `background_tuya_collection`.
...
- In routed-subnet setups, `kasa discover` (UDP broadcast) may return 0; discovery relies on unicast probes over `KASA_SUBNETS`.
- That same routed-subnet layout also means `tinytuya scan` may not see devices from UDP broadcasts alone. Use the cloud-backed wizard to generate `devices.json`, then local polling works across the routed subnet.
